...
class JellyfishCommand(SuperCommand):
    ...
    def run(self):
        ...
        if sys.platform == 'linux':
            ...
            # looks like pip doesn't have a proper way to specify a target
            ...
            # install_name_tool commands were inspired by what was done
            # by reunning delocate (https://github.com/matthew-brett/delocate)
            # on this project's wheel, so that everything happens inside this
            # script
            self.spawn(
                [
                    'install_name_tool',
            ...
            # this command doesn't seem to be essentiel, as per this answer:
